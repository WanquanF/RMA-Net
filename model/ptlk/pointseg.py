...
class PointSEG(torch.nn.Module):
...
#        return r
    ...
    def forward(self, p0, maxiter=10, xtol=1.0e-7):
        f0,f_local0 = self.ptnet(p0)
#        print('#',f0.size(),f_local0.size(),f1.size())
        f0 = f0.view(-1, f0.size()[-1], 1).expand(-1, -1, f_local0.size()[-1])
#        print(f1[0,0,:])
#        exit()
        local_global_feature = torch.cat((f_local0, f0),1)
        
        return f0, f_local0, local_global_feature
